...
class LunarDate extends MyDate {
	...
	private static int checkMonth(int iYear, int iMonth) {
		if ((iMonth > 12)
				&& (iMonth == ChineseCalendar.iGetLLeapMonth(iYear) + 12)) {
			return iMonth;
		} else if (iMonth > 12) {
...
class ChineseCalendar {
	...
	static String[] sHeavenlyStems = { "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛",
			"壬", "癸" };

	static String[] sEarthlyBranches = { "子", "丑", "寅", "卯", "辰", "巳", "午",
			"未", "申", "酉", "戌", "亥" };

	public static String toString(int iYear) {
		int temp;
